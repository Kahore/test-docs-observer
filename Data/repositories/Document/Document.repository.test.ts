...
import {DocumentRepository} from "@/Data/repositories/Document/Document.repository";
import {left, right} from "@/Data/utils";
...
describe('Data/Repositories DocumentRepository', () => {
	let apiClientMock: jest.Mocked<ApiClient>;
	let repository: DocumentRepository;

	beforeEach(() => {
		apiClientMock = new ApiClient( 'myUrl') as jest.Mocked<ApiClient>;
		repository = new DocumentRepository(apiClientMock);
	});
	describe('getDocuments', () => {
		it('getDocuments:: should fetch documents from API', async () => {
			const apiResponse: DocumentResponse = MOCK_1_RESP;
			apiClientMock.get.mockResolvedValue(right(apiResponse)); // Mock API response

			const result = await repository.getDocuments();

			expect(apiClientMock.get).toHaveBeenCalledWith('/documents');
			expect(repository.documents).toStrictEqual(result.value);
			expect(result).toEqual(right(apiResponse));
		});

		it('getDocuments:: should handle API errors', async () => {
			const error = new Error('API Error');
			apiClientMock.get.mockResolvedValue(left(error));
			const result = 	await repository.getDocuments();
